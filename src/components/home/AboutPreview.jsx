...
const AboutPreview = () => {
  ...
  const fadeInUp = { hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.6, ease: 'easeOut' } } };
  const slideFromLeft = { hidden: { x: -100, opacity: 0, scale: 0.95 }, visible: { x: 0, opacity: 1, scale: 1, transition: { duration: 0.8, ease: 'easeOut', delay: 0.3 }}};
  ...
  useEffect(() => {
    if (isInView && yearsCount === 0) {
      ...
      const timer = setInterval(() => {
        ...
        if (currentYear >= targetYears) clearInterval(timer);
      }, interval);
    ...
    <section ref={sectionRef} className="py-24 bg-white relative overflow-hidden">
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-6xl mx-auto">
          ...
          {/* TITLE */}
          <motion.div initial="hidden" animate={isInView ? 'visible' : 'hidden'} variants={fadeInUp} className="mb-16">
            <SectionTitle
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center mt-10">
            ...
            {/* LEFT IMAGE WITH BLACK FRAME */}
            <motion.div initial="hidden" animate={isInView ? 'visible' : 'hidden'} variants={slideFromLeft} className="relative group">
              <div className="bg-black p-3 shadow-2xl"> {/* This creates the black frame */}
                <motion.img
                  whileHover={{ scale: 1.01 }}
                  transition={{ duration: 0.3 }}
                  src="https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?w=900&auto=format&fit=crop"
                  alt="Professional makeup artistry"
                  className="object-cover h-[550px] w-full block"
                />
              </div>
              ...
              <motion.div
                ...
                className="absolute -bottom-6 -right-6 bg-white border border-gray-200 shadow-xl p-5 rounded-xl z-20"
              >
